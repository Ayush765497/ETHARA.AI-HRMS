...
def seed_db():
    ...
    with app.app_context():
        ...
        if not User.query.filter_by(email='admin@ethara.ai').first():
            admin = User(email='admin@ethara.ai', role='admin')
            admin.set_password('admin123')
            db.session.add(admin)
            db.session.flush()
            print("[OK] Admin user created: admin@ethara.ai / admin123")
        else:
        ...
        for name in dept_names:
            ...
            if not dept:
                ...
                db.session.flush()
            depts[name] = dept
        print(f"[OK] Departments ready.")

        # Sample Employees
        sample_employees = [
            ('John', 'Doe', 'john.doe@ethara.ai', 'UI/UX Designer',
             'Design & Marketing', 'Full time', 150000),
            ('Jane', 'Smith', 'jane.smith@ethara.ai',
             'Backend Developer', 'Engineering', 'Full time', 200000),
            ('Alice', 'Johnson', 'alice.j@ethara.ai',
             'HR Manager', 'Human Resources', 'Full time', 180000),
            ('Barry', 'Jonah', 'barry.j@ethara.ai',
             'Frontend Developer', 'Engineering', 'Full time', 160000),
            ('Tiwa', 'Cole', 'tiwa.c@ethara.ai',
        ...
        for first, last, email, title, dept_name, category, salary in sample_employees:
            if not Employee.query.filter_by(email=email).first():
                ...
                if not User.query.filter_by(email=email).first():
                    ...
                    db.session.add(u)
                    emp.user_id = u.id
        print("[OK] Sample employees ready.")
        ...
        print("[OK] Leave types ready.")

        # Announcements
        if not Announcement.query.first():
            db.session.add(Announcement(
                title='Welcome to ETHARA.AI HRMS!',
        ...
        if not Course.query.first():
            ...
            for c in courses:
                db.session.add(c)
        print("[OK] Courses ready.")

        db.session.commit()
        print("\n" + "=" * 50)
        print("  [START] ETHARA.AI HRMS ready!")
        print("  Admin: admin@ethara.ai / admin123")
        print("=" * 50)
